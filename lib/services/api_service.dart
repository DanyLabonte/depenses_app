...
/// Adapter entre le mod?le de lÃ¢â‚¬â„¢app (camelCase + objets riches)
...
class ApiService {
  ...
  /// Ajoute une d?pense (utilis? par ExpenseFormScreen).
  Future<void> addExpense(Expense exp) async {
    final map = _toMap(exp);
    _store.addExpense(map);
  }

  /// (Optionnel) Ajoute plusieurs d?penses en s?quence.
  ...
  /// Historique de lÃ¢â‚¬â„¢utilisateur.
  ...
  /// R?cup?ration brute par ID (attendue par ApprovalDetailScreen).
  Future<Map<String, dynamic>?> findById(int id) async {
    return _store.findById(id);
  }

  /// Variante typ?e mod?le.
  ...
  // ????????????????????????????????????????????????????????????????????????????
  // Compat pour anciens ?crans (HistoryScreen)
  // ????????????????????????????????????????????????????????????????????????????

  /// Ancien nom utilis? dans certains ?crans. Ici on retourne les enregistrements
  /// en attente (N1 + N2) pour ?viter un crash si l'?cran appelle encore ?a.
